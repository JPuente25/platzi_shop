...
const App = () => {
   const { state } = React.useContext(Context); //IMPORTED GLOBAL STATE
   const { loading } = useGetUserSession(state.userTokens?.access_token); //LOADER WHILE CHECKING IF PERSISTED TOKENS ARE OK
   const persistedTokens = JSON.parse(localStorage.getItem('sale_user_session')); //PERSISTED TOKENS
            ...
            <React.Fragment>
               ...
               <Routes>
                  ...
                  <Route
                     ...
                     element={state.userData ? <Checkout /> : <Navigate to="/login" />}
                  />
                  <Route
                     path="*"
                     element={<NotFound />}
